...
export function validateExploreRequest(input) {
  if (!input || !["users", "videos", "profile", "detail", "comments"].includes(input.kind)) throw new ExploreError("invalid_request", "请选择搜索用户或内容。", 400);
  ...
  if (["detail", "comments"].includes(input.kind) && !/^\d{5,30}$/u.test(id)) throw new ExploreError("invalid_request", "请输入有效的作品 ID。", 400);
  return { kind: input.kind, query, id };
}
